...
/**
 * Regenerate a single day (4 slots) and merge with the rest of the week. Fails the day
 * slots to null if no bike fits; caller may toast.
 */
export function buildGenerateDayApplyFlat(
  weekStart: Date,
  dayIndex: number,
  bikes: BikeForGen[],
  hoursByDay: number[][],
  anchorPosts: readonly AnchorPost[],
  loc: LocF,
  preGrid: (SchedulerCell | null)[][],
): ApplyCell[] {
  const base = buildApplyFlat(weekStart, preGrid, hoursByDay);
  const used = new Set<string>();
  for (let d = 0; d < 7; d++) {
    if (d === dayIndex) continue;
    for (let s = 0; s < 4; s++) {
      const c = preGrid[d]![s];
      if (c) used.add(c.bikeId);
    }
  }
  const ordered = interleaveByDealerLocation(
    loc === "all"
  ...
  for (let s = 0; s < 4; s++) {
    ...
    base[start] = null;
    for (let k = 0; k < ordered.length; k++) {
      const idx = (start * 5 + k) % ordered.length;
      const bike = ordered[idx]!;
      if (used.has(bike.id)) continue;
      if (blocked(bike.id, slotT, anchorPosts, localPlaced)) continue;
      localPlaced.push({ bikeId: bike.id, t: slotT });
